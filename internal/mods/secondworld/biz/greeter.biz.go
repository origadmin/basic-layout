...
var (
	...
	ErrUserNotFound = errors.ErrorHTTP(secondworld.HELLO_WORLD_ERROR_REASON_USER_NOT_FOUND, http.StatusNotFound, "user not found")
)
...
func NewGreeterClient(repo dto.GreeterDao, logger log.Logger) secondworld.SecondGreeterAPIClient {
	return &GreeterBiz{dao: repo, log: log.NewHelper(logger)}
}
